...
#stage cost
def lfunc(x,u,p,du):
    return (x-p[:Nx]).T@Q@(x-p[:Nx]) + R*(u-p[Nx:Nu+Nx])**2 + 0.01*du**2
largs = ["x","u","p","Du"]     
...
for t in range(Nsim): #referência para cada instante de tempo
    for k in range(Nt): #referência para o horizonte de predição
            if(t+k > Nsim-1):
                p[k,0] = b[Nsim-1] #y_ref
                p[k,1] = np.arctan2(b[Nsim-1],a[Nsim-1]) #phi_ref
            else:
                p[k,0] = b[k+t] #y_ref
                p[k,1] = np.arctan2(b[t+k],a[t+k]) #phi_ref
            if(t+k < 2):
                phi_ref_plus = np.arctan2(b[t+k+1],a[t+k+1])
                phi_ref_plus2 = np.arctan2(b[t+k+2],a[t+k+2])
