...
a1 = cat_states[:,0,:].T
b1 = cat_controls.reshape((85,2))
b1 = b1[1:,:]
c1 = np.append(0,np.arange(0,round(t[-1][0],1),T))
c1 = np.append(c1,16.6)
...
a2 = cat_states[:,0,:].T
b2 = cat_controls.reshape((85,2))
b2 = b2[1:,:]
c2 = np.append(0,np.arange(0,round(t[-1][0],1),T))
c2 = np.append(c2,16.6)
j = np.around(a1-a2)
h = np.around(b1-b2)
k = np.around(c1-c2)
fig= mpc.plots.mpcplot(a1,b1,c1, xnames = ["x Position","y Position", "Angular Displacement"], unames= ["Velocity","Angular Velocity"])
fig,ax = plt.subplots()
plt.figure(figsize=(5, 2.7))
plt.plot(c2,a2, label='linear')  # Plot some data on the (implicit) axes.
plt.legend()
plt.plot()
plt.show()
mpc.plots.showandsave(fig,"diff.pdf")             
